...
const CATEGORIES = [
    "Nước mắm Phú Quốc", "Cà phê Buôn Ma Thuột", "Bánh pía", "Nem chua", "Kẹo dừa",
    "Trà Thái Nguyên", "Bò một nắng", "Mật ong rừng", "Mè xửng", "Khô cá lóc",
];

const POSTS = Array.from({ length: 15 }, (_, i) => {
    const cat = CATEGORIES[i % CATEGORIES.length];
    const views = Math.floor(500 + Math.sin(i) * 120 + (i % 7) * 80 + (i % 5) * 60);
    return {
        id: `p${String(i + 1).padStart(3, "0")}`,
        title: `Giới thiệu đặc sản ${cat} #${i + 1}`,
        category: cat,
        author: i % 2 === 0 ? "Admin" : "Editor",
        createdAt: new Date(2024, (i * 3) % 12, (i % 27) + 1).toISOString(),
        updatedAt: new Date(2025, (i * 5) % 12, ((i + 10) % 27) + 1).toISOString(),
        views,
    };
});
...
const genYearSeries = (
    year,
    {
        start = 18, goal = 220, seasonAmp = 36, vol = 22, meanRev = 0.35,
        shockProb = 0.22, shockAmp = 110, min = 5, max = 420
    } = {}
    ) => {
    ...
    months.forEach((m) => {
        ...
        if (sign > 0) hasSpike = true; else hasDip = true;
        }
    ...
    if (!hasSpike) {
        const i = 5 + Math.floor(rnd() * 2); // tháng 6-7
        out[i].views = clamp(out[i].views + shockAmp * (0.7 + 0.3 * rnd()));
    }
    if (!hasDip) {
        const j = 9 + Math.floor(rnd() * 2); // tháng 10-11
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 h-[112px]">
                ...
                <div>
                    ...
                    <div className="text-3xl font-bold leading-tight">30.764</div>
                </div>
                </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 h-[112px]">
                ...
                <div>
                    ...
                    <div className="text-2xl font-bold leading-tight">{fmtNumber(POSTS.length)}</div>
                </div>
                </div>
            </div>
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <table className="min-w-full text-sm">
                ...
                {top5.map((p, idx) => (
                    ...
                    <td className="py-2 pr-4 font-semibold">{fmtNumber(p.views)}</td>
