...
const AdminPostDetailPage = () => {
  ...
  useEffect(() => {
    ...
    setBlocks(
      Array.isArray(postDetail.contentBlocks) ? postDetail.contentBlocks : []
    );
    ...
    try {
      ...
      const payload = {
        ...
        contentBlocks: blocks.map((b) => ({
          ...
          caption:
            b.type === "image" ? (b.caption || "").toString() : undefined,
        })),
      };
      await axios.patch(
        `${API_BASE_URL}/statistics/posts/${postDetail.slug}`,
        payload,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
      ...
      setSaveErr(
        err?.response?.data?.message || err.message || "Lưu thay đổi thất bại"
      );
    } finally {
      setSaving(false);
    }
                  ...
                  <div className="space-y-2">
                    <input
                      ...
                      onChange={(e) =>
                        setBlocks((prev) =>
                          prev.map((b, i) =>
                            i === idx ? { ...b, image: e.target.value } : b
                          )
                        )
                      }
                      className="w-full px-3 py-2 rounded border border-gray-200"
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setBlocks((prev) =>
                          prev.map((b, i) =>
                            i === idx ? { ...b, caption: e.target.value } : b
                          )
                        )
                      }
                      className="w-full px-3 py-2 rounded border border-gray-200"
