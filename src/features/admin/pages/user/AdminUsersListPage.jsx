...
export default function UsersListPage() {
  ...
  const params = useMemo(() => ({ page, limit: 20, search: q.trim() || undefined }), [page, q]);
  const { usersList, loading } = useUsersList(params);

  // simple debounce UX
  useEffect(() => {
    const t = setTimeout(() => { /* trigger in hook via params change */ }, 250);
    ...
    <AdminLayout>
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500">Tìm kiếm và xem thông tin nhanh người dùng trên hệ thống</p>
        </div>
      ...
      {/* Search */}
      <div className="mb-6">
        <label htmlFor="user-search" className="sr-only">Tìm người dùng</label>
      ...
      {/* Grid */}
      {loading && (
        <div className="bg-white border border-gray-200 rounded-2xl p-10 text-center text-gray-500">Đang tải…</div>
      )}
      ...
      {/* Pagination (đơn giản) */}
      {!!usersList?.pagination?.totalPages && usersList.pagination.totalPages > 1 && (
        <div className="mt-6 flex items-center justify-center gap-2">
          ...
          <div className="text-sm">
            Trang {usersList.pagination.page} / {usersList.pagination.totalPages}
          </div>
          <button
            disabled={page >= usersList.pagination.totalPages}
            onClick={()=>setPage((p)=>Math.min(usersList.pagination.totalPages, p+1))}
            className="px-3 py-1.5 border rounded disabled:opacity-50"
          >Sau</button>
        </div>
      )}
