...
function StatusBadge({ status }) {
  ...
  const label =
    status === "active"
      ...
      : "Đã cấm";
  return (
    <Pill className={map[status] || "bg-gray-50 text-gray-700 border-gray-200"}>
      {label}
    </Pill>
  );
}
function RoleBadge({ role }) {
  ...
  const label =
    role === "admin"
      ...
      : "Người dùng";
  return (
    <Pill className={map[role] || "bg-gray-50 text-gray-700 border-gray-200"}>
...
export default function AdminUserDetailPage() {
  ...
  const avatarUrl = useMemo(() => {
    ...
    const seed = encodeURIComponent(
      user?.personalInfo?.fullName || user?.email || "U"
    );
      ...
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Quản lý thông tin tài khoản & hồ sơ cá nhân
          </p>
        </div>
        <button
          onClick={() => navigate(-1)}
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <div className="font-semibold">
                    {fmtNumber(Math.round(stats.totalSpend || 0))} ₫
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-2">
        ...
        <div className="xl:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <div className="mb-4">
            ...
            <p className="text-xs text-gray-500">
              Trạng thái, vai trò, xác thực & thời điểm tạo/cập nhật
            </p>
          </div>
          <div className="space-y-3">
            ...
            <InfoRow
              icon={Coins}
              label="Điểm tích lũy"
              value={`${fmtNumber(user.pointsBalance || 0)} điểm`}
            />
            <InfoRow
              icon={CalendarDays}
              label="Tạo lúc"
        ...
        <div className="xl:col-span-3 bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-500">
                Địa chỉ mặc định lấy từ hồ sơ người dùng
              </p>
            </div>
          </div>
