...
export default function AdminDashboard() {
  ...
  const range = mode === "month" ? { year: currentYear, month } : { year: currentYear, quarter };
  ...
  const currencyVN = (n) => new Intl.NumberFormat("vi-VN").format(n ?? 0) + " ₫";
  ...
  const statCards = [
    ...
    { icon: DollarSign, title: "Lợi nhuận", value: currencyVN(summary.profit), delta: "" },
    { icon: Package, title: "Sản phẩm", value: fmtNumber(summary.products), delta: "" },
    { icon: UsersIcon, title: "Người dùng", value: fmtNumber(summary.users), delta: "" },
  ];
    ...
    <AdminLayout>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 xl:col-span-2">
          ...
          <div className="h-[340px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={compare.data} margin={{ left: 20, right: 20, bottom: 24, top: 8 }}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} />
                <XAxis dataKey={compare.xKey} tickMargin={8} padding={{ left: 0, right: 8 }} />
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs text-gray-500">
              {weeklyProfit.weekStart ? new Date(weeklyProfit.weekStart).toLocaleDateString() : ""} - {weeklyProfit.weekEnd ? new Date(weeklyProfit.weekEnd).toLocaleDateString() : ""}
            </span>
          </div>
          <p className="text-sm text-gray-600 mb-2">
            Doanh thu: <b>{currencyVN(weeklyProfit.revenue)}</b> • Lợi nhuận (20%): <b>{currencyVN(weeklyProfit.profit)}</b>
          </p>
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={[
                ...
                <Legend />
                <Bar dataKey="value" name="Giá trị" radius={[6,6,0,0]} barSize={28} fill="#06B6D4" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          ...
          <div className="h-[260px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={topProductsData} layout="vertical" margin={{ left: 20, right: 10 }}>
