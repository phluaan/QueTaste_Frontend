...
const renderCouponType = (type, value, maxDiscount) => {
  switch (type) {
    ...
    case "percentage":
      return `Giảm ${value}%${
        maxDiscount ? ` (tối đa ${maxDiscount.toLocaleString("vi-VN")}₫)` : ""
      }`;
    case "free_shipping":
  ...
  return (
    ...
    >
      {loading.detail ? (
        ...
        <p className="text-que-danger">❌ Lỗi: {error}</p>
      ) : !couponDetail ? (
        <p>Không tìm thấy coupon</p>
      ) : (
        <div>
          {/* Header */}
          <div className="flex items-center justify-between bg-que-background p-4 rounded-lg mb-5">
            <div className="flex items-center gap-3">
              <GiftOutlined className="text-3xl text-que-secondary" />
              <h2 className="text-2xl font-bold text-que-text-main m-0">
                {couponDetail.name}
              </h2>
            </div>
            <Tag color="green" className="text-lg px-4 py-1 rounded">
              {couponDetail.code}
            </Tag>
          </div>
